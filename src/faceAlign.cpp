...
FaceAlign::FaceAlign(IplImage* img,int* rect_C,int* pos_C)
{
	...
	grayimg=cvCreateImage(cvSize(width,height),img->depth,img->nChannels);

	rotaimg=cvCreateImage(cvSize(width,height),img->depth,img->nChannels);

	alignimg=cvCreateImage(cvSize(AlignWidth_A_USTC,AlignHeight_A_USTC),img->depth,img->nChannels);
